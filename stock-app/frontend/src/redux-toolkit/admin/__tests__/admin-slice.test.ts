...
import { mockProductsResponse, perfumeErrorData } from "../../../utils/test/__mocks__/perfumes-mock";
...
describe("admin slice tests", () => {
    ...
    beforeEach(() => {
        ...
        mockFormData.append("perfume", new Blob([JSON.stringify(mockProductsResponse)], { type: "application/json" }));
    });
    ...
    it("should updateProduct dispatches pending and fulfilled on success", async () => {
        ...
        expect(store.getState().perfume.perfume).toEqual(mockProductsResponse);
    });
