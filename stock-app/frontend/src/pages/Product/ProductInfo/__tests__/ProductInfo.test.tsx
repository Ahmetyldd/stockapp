...
describe("ProductInfo", () => {
    it("should render correctly", () => {
        const wrapper = mountWithStore(<ProductInfo perfume={mockFullProductResponse} reviewsLength={11} addToCart={jest.fn()} />);
        expect(wrapper.text().includes(mockFullProductResponse.perfumeTitle)).toBe(true);
        expect(wrapper.text().includes(mockFullProductResponse.perfumer)).toBe(true);
        ...
        expect(wrapper.find(Description).at(7).text().includes(mockFullProductResponse.perfumeGender)).toBe(true);
