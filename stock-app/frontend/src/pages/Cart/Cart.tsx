...
const Cart: FC = (): ReactElement => {
    ...
    const [perfumeInCart, setProductInCart] = useState(() => new Map());
    ...
    useEffect(() => {
        ...
        perfumesFromLocalStorage.forEach((value: number, key: number) => {
            setProductInCart(perfumeInCart.set(key, value));
        });
    ...
    const deleteFromCart = (perfumeId: number): void => {
        ...
        dispatch(removeProductById(perfumeId));
        dispatch(setCartItemsCount(perfumeInCart.size));
    };

    const onChangeProductItemCount = (perfumeId: number, inputValue: number): void => {
        setProducts(perfumeId, inputValue);
        dispatch(calculateCartPrice(perfumes));
    };

    const setProducts = (perfumeId: number, perfumeCount: number): void => {
        setProductInCart(perfumeInCart.set(perfumeId, perfumeCount));
        localStorage.setItem("perfumes", JSON.stringify(Array.from(perfumeInCart.entries())));
    };
