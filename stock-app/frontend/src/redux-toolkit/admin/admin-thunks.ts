...
import { setProduct } from "../perfume/perfume-slice";
import { removeProductById } from "../perfumes/perfumes-slice";
...
export const deleteProduct = createAsyncThunk<{}, number>("admin/deleteProduct", async (perfumeId, thunkApi) => {
    const response = await RequestService.delete(`${ADMIN_DELETE}/${perfumeId}`, true);
    thunkApi.dispatch(removeProductById(perfumeId));
    return response.data;
});
